 ...
 ******************************************************************************
 * @file           : main.c
 * @brief          : LED Pattern Control Application - Main Entry Point
 ******************************************************************************
 * @description
 * This application demonstrates a menu-driven UART interface for controlling
 * LED patterns on STM32F407. It uses FreeRTOS for task management and
 * synchronization.
 *
 * Features:
 * - Interactive UART menu system (115200 baud, 8N1)
 * - Multiple LED patterns controlled by software timers
 * - Thread-safe UART communication using mutex
 * - Idle hook for low-power sleep mode
 *
 * Architecture:
 * - UART Task (Priority 2): Handles character reception and echoing
 * - Command Handler Task (Priority 2): Processes commands from queue
 * - Software Timers: Control LED blinking patterns
 * - Queue: Passes commands between tasks
 * - Mutex: Protects UART transmit operations
 *
 * Hardware:
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	// Creates two software timers for LED pattern control
	led_effects_init();

	// Step 2: Initialize print task subsystem
	// Creates: 1) Print message queue (10 elements × 512 bytes)
	//          2) Print task (priority 3) - owns UART TX exclusively
	print_task_init();

	// Step 3: Initialize UART subsystem
	// Creates: 1) Command queue (5 elements)
	//          2) Command handler task (priority 2)
	uart_task_init();

	// Step 4: Create UART reception task
	// This task handles character-by-character reception and echoing
	// Stack size: 512 words, Priority: 2 (same as command handler)
